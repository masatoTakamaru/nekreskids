/*
sendResizedImg(previewElementID, uploadElementID, sendElementID)
 
previewElementID    string  プレビュー画像を配置するdiv要素のid
uploadElementID     string  ファイルアップロードを配置するdiv要素のid
sendElementID       string  base64文字列を格納するinput要素のID
...
sendResizedImg.create({
    ...
    preview: {
        ...
        imgInit: "/asset/no-image.png", //画像読み込み前の初期画像(ファイル名|base64文字列)
...
class sendResizedImg {
    constructor(previewElementID, uploadElementID, sendElementID) {
        this.previewElementID = previewElementID;
        this.uploadElementID = uploadElementID;
        this.sendElementID = sendElementID;
    }

    create(obj) {
        //初期状態
        const uploadEl = document.getElementById(this.uploadElementID);
        const previewEl = document.getElementById(this.previewElementID);
        const fileEl = uploadEl.appendChild(document.createElement("input"));
        const sendEl = document.getElementById(this.sendElementID);
        ...
        hcvsEl.setAttribute("id", this.sendElementID + "_canvas");
        hcvsEl.setAttribute("width", obj.send.maxWidth);
        hcvsEl.setAttribute("height", obj.send.maxHeight);
        pcvsEl.setAttribute("id", this.previewElementID + "_canvas");
